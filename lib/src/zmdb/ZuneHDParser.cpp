...
std::optional<ZMDBVideo> ZuneHDParser::parse_video(
    ...
    // HD fixed-header layout (see ZMDBVideo doc):
    //   +0x00 folder_ref        +0x18 release_date (FILETIME)
    //   +0x04 title_ref          +0x20 file_size_bytes u32 (HD-only)
    //   +0x08 ref2 (zero)        +0x24 codec_id u16
    //   +0x0c duration_ms        +0x26 playcount u16
    //   +0x10 unknown u32        +0x28 reserved
    //                           +0x2a category u16
    ...
    if (record_data.size() >= 36) video.file_size_bytes = read_uint32_le(record_data, 32);
    if (record_data.size() >= 38) video.codec_id  = read_uint16_le(record_data, 36);
    if (record_data.size() >= 40) video.playcount = read_uint16_le(record_data, 38);
    if (record_data.size() >= 44) video.category  = read_uint16_le(record_data, 42);
    ...
    // In-record UTF-8 title at offset 0x2c — for Series-category videos this
    // is the Episode Title (distinct from title which is the Series Title).
    if (record_data.size() > 0x2c) {
        video.episode_title = read_null_terminated_utf8(record_data, 0x2c);
    }
